...
class MoodleRemoteService @Inject constructor() : ScheduleRemoteService {
     ...
     * 2. Ищет папки с расписанием.
     * 3. Извлекает ссылки на PDF-файлы расписаний.
     *
        ...
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                if (doc.title().contains("Вход") || doc.select(".login-form").isNotEmpty()) {
                    ...
                    Log.d(
                        TAG,
                        "Login token extracted: ${if (loginToken.isNotEmpty()) "YES" else "NO"}"
                    )
                ...
                for (link in folderLinks) {
                    ...
                    val isExam = text.contains("Экзамен", ignoreCase = true) || text.contains(
                        "Зачет",
                        ignoreCase = true
                    )
                    val isGroupList = text.contains("Списки групп", ignoreCase = true)

                    if ((text.contains("Расписание", ignoreCase = true) || text.contains(
                            "курс",
                            ignoreCase = true
                        ))
                        && !isExam && !isGroupList
                ...
                for (folderUrl in relevantFolders) {
                    try {
                        ...
                        for (link in fileLinks) {
                            val href = link.attr("href")
                            if (href.contains("forcedownload=1") || href.contains("/mod/resource/view.php") || href.contains(
                                    "pluginfile.php"
                                )
                            ) {
